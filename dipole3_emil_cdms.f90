...
!     all data etc for the bra are labelled 2.
      ...
      double precision, allocatable, dimension(:) :: dstemp, dc1, dc2
...
!     read in radial dvr grid points (same for bra and ket)
	...
	write(*,*) 'Printing r1 quadrature points'
	write(6,*) r1
      if (idia .gt. -2) call getrow(r2,npnt2,ibra)
write(*,*) 'Printing r2 quadrature points'
	write(6,*) r2
...
!------------------
 ...
 2020   format(/5x,'Block',i4,' k1 =',i3,' to k2 =',i3,' skipped')
         ...
         if (idia.eq.-2 .and. mod((kk+ipar11)/2+(kk+ipar22)/2,2).ne.0) xfac=-xfac
      ...
      if (k2.le.jk2) then
       ...
       if (jblock.gt.iblock) then
        ...
        if (nbass2(k2).eq.0) then
         ...
         call trans(tx,dipol,binom,dc1,dc2,k1,k2,xfac,nu,ip)
         call wrscr(tz,tx,neval1*neval2,iscr,iblock)
         write(6,2000) iblock,k1-kmin1,k2-kmin2
         if (iblock .ge. kblock) goto 154
        endif
      ...
      if (k2.ge.1) then
       ...
       if (jblock.gt.iblock) then
        ...
        if (nbass2(k2).eq.0) then
         ...
         xfac= x1/sqrt(x2)
         if (kk2.eq.0) xfac= x1
         if (idia.eq.-2 .and. mod((kk1+ipar11)/2+(kk2+ipar22)/2,2).ne.0) xfac=-xfac
         if (.not. zembed .and. idia .lt. 0) xfac=-xfac
         call dsrd(dc2,dstemp,ibra,mbass2,nbass2(k2),neval2,&
                   k2,kbeg2,jk2,ip,ibase2,xd,kk2,nu,ipar2)
         call trans(tx,dipol,binom,dc1,dc2,k1,k2,xfac,nu,ip)
...
203   format(5x,'j1=',i4,'  kmin1=',i4,'  j2=',i4,'  kmin2=',i4,&
      ...
      ezero=x0
      read(5,*) ezerolow,ezeroup,te
  505 format(f20.0)
      write(6,204) ezerolow
204   format(5x,'electronic ground state ZPE =',e16.8,' cm-1')
      write(6,299) ezeroup
299   format(5x,'electronic excited state ZPE =',e16.8,' cm-1')
      write(6,374) te
374   format(5x,'Excitation energy (0->0) =',e16.8,' cm-1')
  555 continue
...
!Emil: Here we shift energy levels by Te. New levels are printed to screen and passed to outpt, which generates input for SPECTRA


      do 1 ie1=1,neval1
      xe1= e1(ie1)*autocm-ezerolow

      do 2 ie2=1,neval2

      if (ie1.eq.1) xe2(ie2)= e2(ie2)*autocm+te-ezeroup
      dd= xe2(ie2) - xe1
      ...
      write(6,206) ie1,ie2,xe1,xe2(ie2),dd,tzd,txd,t,sxd,a ! Saving - screen output
206   format(2(i4),3(3x,f14.3),5(2x,e13.6))
2     continue
...
! then it will write the values of sint ( in au as weel)
      ...
      if (ztra) call outpt(xe1,xe2,sint) ! Saving - input for spectra
      return
      end
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
!                                                **026
      subroutine outpt(e1,e2,sint)
