...
!     Rewritten into fortran 95 by Max Kostin and Jonathan Tennyson
      implicit logical (z)
      common /outp/ zpham,zprad,zpvec,zrot,zladd,zembed,zmors2,&
      ...
      namelist/prt/ zpham,zprad,zpvec,zrot,zladd,zembed,zmors2,&
                    zpmin,zvec,zquad2,zdiag,zlmat,zcut,zall,zlin,&
...
!     tswalf is the exact sum of weights for gauss-jacobi integration
   ...
   30 continue
      ...
      if(.not. zperp) then
        ...
        write(512,1031) (xalf(ii),ii=1,nang2)
 1031   format(//f23.15) 
...
!########################################################################
      ...
      double precision, dimension(nham3*3) :: work3
...
!     hamiltonian.
      ifail=0
      nnham=3*nham3
      call dsyev('V','L',nham3,ham3,nham3,eval,work3,nnham,ifail)
...
!     write the final vectors to disk if required
 ...
 1070 format(//'  Band origins in wavenumbers:'/)
      write(6,1021) (work3(i)-ezero,i=ii,meval)
 1021 format(5f15.6/)
...
!     set psi to zero
  ...
  458     continue
          ...
          do ia=1,npnt2
          do ib=1,npnt1
          write(503,*)ic,ia,psi(ic,ib,ia)**2
          end do
          end do
          write(503,*)
          end do
