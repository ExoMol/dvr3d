      SUBROUTINE DIPD(DIPC,R1_bohr,R2_bohr,XCOS,NU)
C
...
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Y), LOGICAL (Z)
        dimension cart(1,3,3)
        dimension xyz(3,3),dip(3)
      double precision :: xpx(1:300),xpy(1:300)
      integer :: i_t,npropinx,npropiny
...
C
      DATA X1/1.0D0/,X0/0.0D0/,TINY/9.0D-15/,X2/2.0D0/,PI/3.1415927D0/
         ...
         pi=dacos(-1.d0)
         R1=R1_bohr*0.5291772d0
         R2=R2_bohr*0.5291772d0
...
C        GENERAL COORDINATES (INCLUDING RADAU): ATOM 1 = ATOM 2
      ...
      ENDIF
        ...
        xyz=0.d0
        dip=0.0d0
        xyz(2,1)=Q1*cos(theta/2)
        xyz(2,2)=Q1*sin(theta/2)
        xyz(3,1)=Q2*cos(-theta/2)
        xyz(3,2)=Q2*sin(-theta/2)
        cart(1,:,:)=xyz(:,:)

      Q1_ang = Q1!*0.5291772d0
      Q2_ang = Q2!*0.5291772d0

!      call potv(V,R1_bohr,R2_bohr,XCOS)
        
! Read coeffs for muX
      open(unit=32,status='old',form='formatted',file="dipx.fit")
      read(32,*) npropinx ! Read the number of DMSx coefficients
      do i = 1,npropinx
         read(32,'(A10,I8,ES22.6)') label,i_t, xpx(i)
      end do

      close(unit=32)

! Read coeffs for muY
      open(unit=32,status='old',form='formatted',file="dipy.fit")
      read(32,*) npropiny ! Read the number of DMSx coefficients
      do i = 1,npropiny
         read(32,'(A10,I8,ES22.6)') label,i_t, xpy(i)
      ...
      close(unit=32)
       ...
       CALL DIPS(DIPX,xpx,npropinx,DIPY,xpy,npropiny,Q1,Q2,theta)
       dip_temp=DIPX
       DIPX=DIPY
       DIPY=dip_temp
C
...
C
      ...
      ENDIF
       ...
       IF (theta*180.0/pi .gt. 145.0 .or. theta*180.0/pi .lt. 80.0) then
 	DIPC=0.0000
 	endif
