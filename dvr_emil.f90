...
!     Rewritten into fortran 95 by Max Kostin and Jonathan Tennyson
      ...
      write(6,1000)
 1000 format(5x,'Program DVR3DRJZ (version of November 2002)')
...
!     intermediate and the final hamiltonians.
      ...
      REAL*8, ALLOCATABLE, DIMENSION(:,:) :: angular_quad
     ...
     ALLOCATE(dnorm1(0:nmax1),r1m2(nlim1),dnorm2(0:nmax2),&
               ...
               ,xk2(npnt2,npnt2),r2m2t(npnt2,npnt2),r1m2t(npnt1,npnt1),angular_quad(nalf,jrot+1))
...
!     for j > 0, store r**(-2) term for rotlev3
      if (ztran) then
         ...
         open(45,file='radial_quad.dat')
         write(45,'(ES15.6)') (r1(i), i=1,npnt1)
         close(45)
         write(iwave) r1
...
!     tswalf is the exact sum of weights for gauss-jacobi integration
	...
	!call gasleg(nang,nang2,xalf,walf,realkz,realkz,cswalf,tswalf)
...
!     save polinomials for rotlev3 or rotlev3b, or rotlev3z
      if (ztran) then
        ...
        open(44,file='angular_quad.dat')
        write(44,'(ES15.6)') xalf
        close(44)
      
       do i=1,nalf
        angular_quad(i,kk)=xalf(i)
       end do 
        write(iwave) xalf
        write(iwave) kz,maxleg,nidvr,lincr
        write(iwave) ((pleg(i,j)*walf(j),i=0,maxleg),j=1,nidvr)
        !open(46,file='T_ang.dat')
        !write(46,'(<maxleg+1>ES16.6)') ((pleg(i,j)*walf(j),i=0,maxleg),j=1,nidvr)
        !close(46)
...
!     driving routine
      if (idia .le. -2) then
         ...
         if (zperp) then ! Call main
           call mkmain(xk1,r1m2t,xlmatr,r1,xalf,kz)
         else
           call nfmain(xk1,r1m2t,xlmatr,r1,xalf,kz) ! Radau symmetrized, kz=k
         endif    
      else
...
!     symmetrised radau coordinates. IDIA=-1
...
!     print eigenvalues in atomic untis & wavenumbers
      ...
      if (.not.zpmin) then
      write(6,'(5x, A, I15, A)')   &
     &   'Lowest ',meval,' eigenvalues in hartrees:'
...
!     save the eigenvalues if needed
   ...
   20 continue
      write(6,'(/, 5x, A, I15, A)')  &
     &   'Lowest ',meval,' eigenvalues in wavenumbers:'
...
!     write the final vectors to disk if required
 ...
 1061    format((2I6,d20.10))
   ...
   55    continue
      ...
      write(6, '(/,5x,  A)') '  Band origins in wavenumbers:'
...
!####################################################################
      ...
      subroutine gasleg(nn,nn2,x,a,alf,bta,csa,tsa)
...
!    IDIA=-2 this routine controls the dvr calculation in the case of
...
do 30 igamma = 1,nalf ! For each angular point separately build 2D Hamiltonian
  
        nv2(igamma) = (npnt*(npnt+1-ipar*2))/2
        nham2 = nv2(igamma)
        call blc2d1(theta(igamma),r,hr,ham2,nham2,& 
                          term,term2,term3,kz) ! build 2D Hamiltonian using hr=(T*hblT), return ham2, nham= size of 2D hamiltonian - this is done for every angle separately (2D problem is defined on radial coorinates only)


        call diag_dac(ham2,nham2,nham2,eig2) ! Diagonalize ham2, store energies in eig2


        if (.not. zcut) then
          call choosr(igamma,nham2,eig2,ham2,iv2,eigs2d,& 
                      vecs2d,nv2,eigtmp)  ! Choose lowest MAX3D dolutions. Output: truncated matrices and vectors: eigs2d,vecs2d. Chooses eigenvalues between eigs2(1) and emax.

          if(igamma .eq. nalf .and. .not. zpmin)& 
              write(6,110) (itmp, iv2(2,itmp),itmp=1,nalf) ! print how many energies are kept for a given gamma point
        else
           call cut2dr(igamma,nham2,eig2,ham2,iv2,eigs2d,vecs2d) ! Selects all energies below emax, truncates matrices and vectors
          if (.not. zpmin) write(6,110) igamma, iv2(2,igamma) ! print how many energies are kept for a given gamma point
        endif
...
30    continue
      ...
      nham3 = iv2(1,nalf) + iv2(2,nalf) - 1 ! how large is our 3D Hamiltonian, iv(1,nalf) is probably the largest value in iv2 vector (cumulative) representing total number of solutions to include
      
      if (.not. zpmin) write(6,120) nham3 ! yes, this is the number of solutions declared as max3d
      call nftim('end of 2d loop')
      write(6,160)
      ...
      call diag3d(ham3,nham3,eig3,kz)  ! Diagonalisation

      call nftim('end of diagonalising 3d')

      if (ztran) call transr(iv2,vecs2d,ham3,eig3,nham3,nbass) !Saving eigenvectors

      DEALLOCATE(iv2,vecs2d,ham3,eig3)
      return
110   format(5x,'for gamma = ',i3,' selected ',i3,' energies.')
120   format(25x,'total = ',i5)
130   format(5x,'starting the 2d calculations.')
160   format(5x,'building the 3d hamiltonian.')
170   format(5x,'diagonalising the 3d hamiltonian.')
        ...
        do 20 ibeta=1,npnt
          ...
          do 30 ialphp=1,ibetap-ipar 
            ...
            do 40 ialpha=1,ibeta-ipar
              ...
              if(ibeta .eq. ibetap) ham2(ialphp+iap,ialpha+ia)=ham2(ialphp+iap,ialpha+ia)+hr(ialphp,ialpha)
              if(ibeta .eq. ialphp) ham2(ialphp+iap,ialpha+ia)=ham2(ialphp+iap,ialpha+ia)+q*hr(ibetap,ialpha)
              if(ialpha .eq. ibetap) ham2(ialphp+iap,ialpha+ia)=ham2(ialphp+iap,ialpha+ia)+q*hr(ialphp,ibeta)
              if(ialpha .eq. ialphp) ham2(ialphp+iap,ialpha+ia)= ham2(ialphp+iap,ialpha+ia)+hr(ibetap,ibeta)
              
              if(ialpha .eq. ialphp .and. ibeta .eq. ibetap) then
                call potv(v,r(ibeta),r(ialpha),xcos)
                ham2(ialphp+iap,ialpha+ia)=ham2(ialphp+iap,ialpha+ia)+v
                
              endif
              
              if(ialpha .eq. ibetap .and. ibeta .eq. ialphp) then
                call potv(v,r(ibeta),r(ialpha),xcos)
                ham2(ialphp+iap,ialpha+ia)=ham2(ialphp+iap,ialpha+ia)+q*v
              endif
              
              if(ialphp .eq. ibetap) ham2(ialphp+iap,ialpha+ia)=ham2(ialphp+iap,ialpha+ia)*factr2
              if(ialpha .eq. ibeta) ham2(ialphp+iap,ialpha+ia)=ham2(ialphp+iap,ialpha+ia)*factr2
      ...
      do 10 ialpha = 1,nham2
        ...
        if (eig2(ialpha) .lt. emaxau) then
         	 ntot = npos + nvec
          		...
          		if (ntot .gt. max3d) then !nham2 must be greater than our truncation params max3d
...
!!              wavefunction has amplitude all the way to theta = 0: stop
  ...
  987    format(/5x,'*** warning: zlin = t forced the removal of',i4,&
      ...
      if (ztran) then
         ...
         write(iwave) ((iv(jj,ii),ii=1,nalf),jj=1,2)
      endif
      return
      end
...
!***********************************************************************
      ...
      real*8, dimension(nham3,nham3) ::  ham3
      real*8, dimension(max3d) :: eigs2d
      real*8, dimension(max2d,max3d) :: vecs2d !TRuncated Ham2d
      dimension iv2(2,nalf),nv2(nalf)
      real*8, dimension(max2d,max2d) :: ham2 ! 2D hamiltonian transition matrix - original, not truncated. Diagonalizes Ham2D
      real*8, dimension(npnt) :: r ! Vector of radial quadrature points (NPNT)
...
!     ***********************************************************************
      ...
      subroutine transr(iv2,vecs2d,ham3,eig3,nham3,nbass) !Subroutine to store wavefunctions and eigenvalues
      implicit real*8(a-h,o-y),logical(z)
      common /size/ npnt1,npnt2,nalf,nmax1,nmax2,maxleg,nalf2,idvr,&
                    npnt,nlim1,nlim2,neval,ncoord,&
                    jrot,kmin,idia,ipar,&
      ...
      real*8, dimension(max2d,max3d) :: vecs2d
      real*8, dimension(nham3,nham3) :: ham3
      real*8, dimension(nham3) :: eig3 !nham3=neval
      real*8, dimension(nbass) :: wvfunc
      real*8, dimension(nbass) :: wvfuncdvr !our wavefunction
      ...
      write(6,109) iv2
       write(iwave) iv2
      open(66,file='iv2.dat')
      write(66,109) iv2
      close(66)

do 40 level3=1,neval !Main loop (for wavefunctions) over solutions (number of solutions is determined by global neval parameter read from input file)
        index=1
        wvfunc = 0.0d0
        wvfuncdvr = 0.0d0
        
       do 30 irs=1,nrad ! Radial points loop
       
          do 300 igamma=1,nalf  ! Angular points loop
          
                  if (iv2(2,igamma) .eq. 0) goto 300! Skip saving if iv2 is 0 for this igamma - truncation of angular points
            
            	  do 20 level2=0,iv2(2,igamma)-1 ! select iv2(2,igamma) solutions for a given igamma (angular coordinate). In order to get a solution (wf) in a point, we multiply 3-D Ham transformation matrix by eigenvectors of 2D Ham - this gives expansion of 3D solutions
            	
             		 ivec2=iv2(1,igamma)+level2	 ! iv2(1,gamma) must be a cumulative index
              		 wvfuncdvr(index)=wvfuncdvr(index)+vecs2d(irs,ivec2)*ham3(ivec2,level3) 
              		 
				  20          continue
				
              index=index+1 ! So igamma comes first and is constrained by iv2(2,igamma)>0, hence not all points land in the vector. Then comes irs - radial loop.
  
			300       continue  ! Angular points loop
			
		30      continue  ! Radial points loop
		
        write(iwave) wvfuncdvr ! Write to fort.26
